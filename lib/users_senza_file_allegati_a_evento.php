...
class CapoGita extends Associato {
	...
	function iscritti()
	{
		...
		if(is_numeric($id))
		{
			include_once("lib/db_mysql.php");
			$db = new db_local();
			if($db->query("SELECT gite.titolo,iscrizioni.*,anagrafiche.nome,anagrafiche.cognome,anagrafiche.tel1,anagrafiche.email,anagrafiche.cauzione FROM iscrizioni,anagrafiche,gite WHERE anagrafiche.id = iscrizioni.idassociato AND gite.id = iscrizioni.idgita AND gite.id = '".$id."' AND iscrizioni.idresp <> CONCAT(iscrizioni.idassociato,'-NS') AND(gite.idcreat = '".$this->matricola."' or gite.idresp = '".$this->matricola."' or ".$this->matricola." = 0 or '".$this->carica."' = 'A') ORDER BY anagrafiche.cognome,anagrafiche.nome;"))
...
if(is_numeric($_GET['list']))
	...
	function listmail()
	{
		...
		$sqlqry = "select email.*,gruppimail.nome from email inner join gruppimail on email.idgruppo = gruppimail.id";
		if($db->query($sqlqry))
		{
		?>
			<br>
