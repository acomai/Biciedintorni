...
$idgruppo = $db->record['id'];

?>
<div align="center" id="title"><h2>Modifica Gruppo</h2></div>
<div align="center" style="color:red">I campi con * sono obbligatori.</div>
<form id="frmmodgruppo" enctype="multipart/form-data" method="post" name="modgruppo" action="admin.php?fun=modgruppo&save=1">
  <table id="Tmodgruppo" align="center" style="text-align: left;" border="1" cellpadding="2" cellspacing="2">
    <tbody>
	  <tr>
        <td id="tdnome" class="title">Nome gruppo*</td>
        <td><input id="nome" maxlength="255" size="100" name="nome" value="<?php echo $db->record['nome']; ?>"></td>
      </tr>
      <tr>
        <td id="tddescri" class="title">Descrizione gruppo*</td>
        <td><input id="descri" maxlength="4000" size="100" name="descri" value="<?php echo $db->record['descrizione']; ?>"></td>
      </tr>
      <tr>
		...
		<script>
        	...
        	<table width="100%">
        		<tr>
					...
					$db->query("INSERT INTO TMPSelezionati".$numtable." (id) (Select idsocio from lnk_gruppi_soci where idgruppo = ".$idgruppo.");");
					$db->query("SELECT * FROM anagrafiche where id > 0 and id not in (select id from TMPSelezionati".$numtable.") and (email is not null and trim(email) <> '') order by cognome, nome ;");
        			...
        			<td width="47%" style="vertical-align:top;"><div id="listaselezionati" style="height:400px;overflow:auto;padding-left:2px;vertical-align:top;">
						<table width="100%">
							...
							<?php
							$db->query("SELECT id,cognome,nome,email FROM anagrafiche where id in (select id from TMPSelezionati".$this->matricola.") order by cognome,nome;");
							while($db->next_record())
							{?>
	  ...
	  <tr>
		<td colspan="2" align="right"><input type="hidden" value='<?php echo $idgruppo; ?>' name="id"><button type="button" onclick="controllaFormGruppo();">Invio</button></td>
  	  </tr>
    </tbody>
  </table>
</form>
