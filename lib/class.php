...
function vis_cart()
{
	if(is_numeric($_GET['id']))
	{
		...
		if($db->query("SELECT * FROM cartine WHERE id = '".$_GET['id']."' LIMIT 1;"))
			require_once("lib/html/viscart.php");
		else 
...
function vis_libro()
{
	if(is_numeric($_GET['id']))
	{
		...
		if($db->query("SELECT * FROM libri WHERE id = '".$_GET['id']."' LIMIT 1;"))
			require_once("lib/html/vislibro.php");
		else 
	...
	if(is_numeric($_GET['iscr']))
	{
		...
		if($db->next_record())
		{
			...
			if (((date("Y") == date("Y",$db->record['data'])) && (date("n") == date("n",$db->record['data'])) && ((date("j",$db->record['data']))-(date("j")) <= 1)) OR (($numisc >= $db->record['maxp']) && ($db->record['maxp'] > 0)))
			{
				...
				echo "<br><br><div id=\"msg\" align=\"center\" style=\"font-size:16px; color: #FF0000\">Le iscrizioni alla gita sono terminate o � stato raggiunto il numero massimo di partecipanti. Contattare il capogita.</div>\n";
