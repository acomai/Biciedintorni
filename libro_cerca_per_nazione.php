...
<head>
  <title>FIAB Torino Bici e Dintorni - libro_cerca_per_nazione.php</title>
  <meta charset="utf-8">
...
/**
 * Libro_cerca_per_nazione.php File Doc Comment
 *
 * PHP version 5.3
 * Programma che fornisce l'elenco dei libri nel db di
 * Bici e Dintorni che fanno riferimento alla nazione cercata
 *
 * @category Programma
 * @package  Root
...
$sql = "SELECT id, titolo, anno, descrizione, classificazione FROM libri WHERE idnazione = $nazione ORDER BY titolo;" ;

$result = $conn->query($sql);

$sql2 = "SELECT nome FROM nazioni WHERE id = $nazione;" ;

$result2 = $conn->query($sql2);

while ($row2 = $result2->fetch_assoc()) {
	$descNazione = $row2["nome"];
}

echo "<div class='row'>";
	echo "<div class='col-xs-12'>";
		echo "Nazione cercata: " .  $nazione . " - " . $descNazione; 
	echo "</div>";
echo "</div>";
echo "<hr />";
